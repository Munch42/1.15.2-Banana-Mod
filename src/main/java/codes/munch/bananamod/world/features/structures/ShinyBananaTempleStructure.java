...
public class ShinyBananaTempleStructure extends Structure<NoFeatureConfig> {
    public ShinyBananaTempleStructure(Function<Dynamic<?>, ? extends NoFeatureConfig> config){
        super(config);
    }
    ...
    {
        //this means Run Down Houses (My Shiny banana temple) cannot be closer than 7 chunks or more than 12 chunks
        int maxDistance = 50;
        int minDistance = 10;
    ...
    {
        return BananaMod.MOD_ID + ":shiny_banana_temple";
    }
    ...
    {
        return ShinyBananaTempleStructure.Start::new;
    }
    ...
    public static class Start extends StructureStart
    {
        ...
        public void init(ChunkGenerator<?> generator, TemplateManager templateManagerIn, int chunkX, int chunkZ, Biome biomeIn)
        {
            ...
            BlockPos blockpos = new BlockPos(x, surfaceY - 4, z);

            //Now adds the structure pieces to this.components with all details such as where each part goes
            //so that the structure can be added to the world by worldgen.
            ShinyBananaTemplePieces.start(templateManagerIn, blockpos, rotation, this.components, this.rand);

            //Sets the bounds of the structure.
            this.recalculateStructureSize();

            //I use to debug and quickly find out if the structure is spawning or not and where it is.
            //BananaMod.LOGGER.log(Level.DEBUG, "Rundown House at " + (blockpos.getX()) + " " + blockpos.getY() + " " + (blockpos.getZ()));
        }
